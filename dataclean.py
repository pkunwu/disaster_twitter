...
#check the coverage of word-embedding
def build_vocab(X):
...
def check_embeddings_coverage(X, embeddings):
    ...
    sorted_not_covered = pd.DataFrame(sorted(not_covered.items(), key=lambda x:x[1], reverse=True))
    return sorted_not_covered, vocab_coverage, text_coverage

def embedding_rs(df,col):
    glove_embeddings = np.load('../NLP/glove.840B.300d.pkl', allow_pickle=True)
    fasttext_embeddings = np.load('../NLP/crawl-300d-2M.pkl', allow_pickle=True)

    train_glove_not_covered, train_glove_vocab_coverage, train_glove_text_coverage = check_embeddings_coverage(df[col], glove_embeddings)
    print('GloVe Embeddings cover {:.2%} of vocabulary and {:.2%} of text in Training Set'.format(train_glove_vocab_coverage, train_glove_text_coverage))

    train_fasttext_not_covered, train_fasttext_vocab_coverage, train_fasttext_text_coverage = check_embeddings_coverage(df[col], fasttext_embeddings)
    ...
    return train_glove_not_covered, train_fasttext_not_covered
# test
# train_glove_not_covered, train_fasttext_not_covered = embedding_rs(df,'text')
...
# test
# train_glove_not_covered, train_fasttext_not_covered = embedding_rs(df, 'text_cleaned')

# relabel, since there are mislabels.
